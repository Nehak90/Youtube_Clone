...
async function fetchVideoDetails(videoId) {
    ...
    videoDiv.addEventListener('mouseover', e => playVideoPreview(videoId, videoDetails.snippet.thumbnails.high.url))
    videoDiv.addEventListener('mouseout', e => stopVideoAndDisplayThumbnail(videoId))
    ...
    videoDiv.innerHTML = `
        ...
        <img class="channel-logo"
            ...
            alt="">
        <div class="details">
            <h3 id="title">${videoDetails.snippet.title.slice(0, 64) + "..."}</h3>
...
function playVideoPreview(videoId, currrentThumbnail) {
    ...
    if (YT) {
        ...
        document.getElementById('thumbDiv-' + videoId).addEventListener('click', e=> {
            e.preventDefault()
            goToDetailsPage()
        });
    }
