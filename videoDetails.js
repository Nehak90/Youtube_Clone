...
async function fetchAndShowVideDetails() {
    const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?key=${API_KEY}&part=snippet,statistics&id=${videoId}`)
    const data = await response.json()
    const videoDetails = data.items[0]

    const channelResponse = await fetch(`${BASE_URL}/channels?key=${API_KEY}&part=snippet,statistics&id=${videoDetails.snippet.channelId}`);
    ...
    description.addEventListener('click', (e) => { e.target.innerText = descriptionFullText })
    likeCount.innerText = formatNumber(videoDetails.statistics.likeCount)
    subCount.innerText = formatNumber(channelData.items[0].statistics.subscriberCount) + " subscribers"
...
async function renderComments() {
    const response = await fetch(`${BASE_URL}/commentThreads?key=${API_KEY}&videoId=${videoId}&maxResults=30&part=snippet`);
    const data = await response.json();
    ...
    commentData.map(el => {
        ...
        comment.innerHTML = `
        <img src="${el.snippet.topLevelComment.snippet.authorProfileImageUrl}" alt="">
        <div>
        <h5>${el.snippet.topLevelComment.snippet.authorDisplayName}</h5>
        <p>${el.snippet.topLevelComment.snippet.textOriginal}</p>
        ${el.snippet.totalReplyCount > 0 ? `<b id="${el.snippet.topLevelComment.id}" onclick="loadReplies(event)" >${el.snippet.totalReplyCount} replies</b>` : ''}
        </div>
        `
...
async function loadReplies(e) {
    ...
    const response = await fetch(`${BASE_URL}/comments?key=${API_KEY}&parentId=${commentId}&part=snippet&maxResults=10`)
...
async function fetchVideos(searchTerm, maxResult) {
    const response = await fetch(`${BASE_URL}/search?key=${API_KEY}&q=${searchTerm}&maxResults=${maxResult}&part=id&type=video`)
    const data = await response.json()
...
async function fetchVideoDetails(videoId) {
    ...
    const channelResponse = await fetch(`${BASE_URL}/channels?key=${API_KEY}&part=snippet&id=${videoDetails.snippet.channelId}`);
    const channelData = await channelResponse.json();
    ...
    videoDiv.innerHTML = `
        <div class="video-card-img" id="thumbDiv-${videoId}">
            <img id="thumbImg-${videoId}" src="${videoDetails.snippet.thumbnails.high.url}" alt="">
         </div>
        <div class="video-card-details">
        <div class="details">
            <h3 id="title">${videoDetails.snippet.title.slice(0, 40) + "..."}</h3>
            <p>${videoDetails.snippet.channelTitle}</p>
            <p><span>${formatViewCount(videoDetails.statistics.viewCount)}</span> views <span>${calculateTimeGap(videoDetails.snippet.publishedAt)}</span></p>
        </div>
    `
